...
export function createSupabaseServices(config: AppConfig): AppServices {
  ...
  return createSupabaseServicesFromClient(supabase);
}
  ...
  const services: AppServices = {
    ...
    organization: {
      async createOrganizationWithAdmin(input) {
        ...
        const admin = await services.auth.createUser({
        ...
        await services.profile.upsertProfile({
          id: admin.id,
          email: admin.email,
      ...
      async getUsedSeats(organizationId: string) {
      ...
      async isOrganizationAdmin(userId: string, organizationId: string) {
        const { data, error } = await supabase
          .from("organization_memberships")
          .select("id")
          .eq("user_id", userId)
    ...
    ranking: {
      async getSummary({ examSlug, profile }) {
        const selectColumns =
